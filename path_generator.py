...
from generate_samples import generate_samples, format_obstacles
...
def main(cx, cy, obstacle_path, model, start=None, goal=None, num_evals=1, eval_mode=False, plotopt=False):
    ...
    """
    object_c = format_obstacles(load_polys=obstacle_path)
    valid_counter = 0
    path_set = []
    for i in range(num_evals):
        ...
        s_pred_var = Variable(torch.from_numpy(s_pred)).cuda().float()
        goal_var = Variable(torch.from_numpy(goal_ext)).cuda().float()
...
if __name__ == '__main__':
    ...
    _, validity = main(cx, cy, obstacle_path, model, num_evals=1, eval_mode=True, plotopt=True)
